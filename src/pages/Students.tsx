...
type GradesSubTab = 'input' | 'trend' | 'rank';
...
function GradeRank({ students, classes, scores }: { students: Student[]; classes: string[]; scores: ExamScore[] }) {
  const [selectedClass, setSelectedClass] = useState(classes[0] ?? '');
  ...
  const classStudents = students.filter(s => s.className === selectedClass);
  const examNames = [...new Set(scores.filter(s => s.className === selectedClass).map(s => s.examName))].sort();

  useEffect(() => {
    if (examNames.length > 0 && !examNames.includes(examName)) setExamName(examNames[0]);
  }, [selectedClass, examNames.join('|')]); // eslint-disable-line react-hooks/exhaustive-deps

  const rows = classStudents.map(stu => {
    const subjScores = scores.filter(s => s.studentName === stu.name && s.className === selectedClass && s.examName === examName);
    const avg = subjScores.length > 0 ? subjScores.reduce((sum, s) => sum + scorePct(s), 0) / subjScores.length : null;
    return { student: stu, avg };
  });

  const ranked = rows
    .filter((r): r is { student: Student; avg: number } => r.avg !== null)
    .sort((a, b) => b.avg - a.avg);
  const unranked = rows.filter(r => r.avg === null);

  let rank = 0, prevAvg: number | null = null;
  const rankedWithPos = ranked.map((r, i) => {
    if (prevAvg === null || Math.abs(r.avg - prevAvg) > 0.001) rank = i + 1;
    prevAvg = r.avg;
    return { ...r, rank };
  });
  ...
  return (
    <div className="space-y-4 max-w-2xl">
      <div className="flex gap-2">
        ...
        <button type="button" onClick={() => setSubTab('rank')}
          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
            subTab === 'rank' ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
          }`}>
          <Trophy size={13} /> 평균/순위
        </button>
      </div>
      ...
      {subTab === 'trend' ? (
        ...
        <GradeRank students={students} classes={classes} scores={scores} />
      ) : (
        <>
