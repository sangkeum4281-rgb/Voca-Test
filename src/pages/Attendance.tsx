...
import { Loader, ChevronLeft, ChevronRight, RotateCcw, CalendarDays, CalendarCheck, CalendarRange, Printer, AlertTriangle } from 'lucide-react';
...
const ATTENDANCE_ALERT_THRESHOLD = 5;
...
export default function Attendance() {
  ...
  // 지각·결석 합계 5회 이상 학생 (전체 반 기준)
  const flaggedStudents = students
    .map(s => {
      const sMap = monthlyByStudent[s.name] ?? {};
      const lateCnt = Object.values(sMap).filter(v => v === 'late').length;
      const absentCnt = Object.values(sMap).filter(v => v === 'absent').length;
      return { ...s, lateCnt, absentCnt, total: lateCnt + absentCnt };
    })
    .filter(s => s.total >= ATTENDANCE_ALERT_THRESHOLD)
                ...
                <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <h3 className="font-semibold text-amber-800 text-sm">
                      지각·결석 {ATTENDANCE_ALERT_THRESHOLD}회 이상 학생 ({flaggedStudents.length}명)
                    </h3>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {flaggedStudents.map(s => (
                      <div key={s.id} className="flex items-center gap-2 bg-white border border-amber-200 rounded-lg px-3 py-2 text-sm">
                        <span className="font-medium text-slate-800">{s.name}</span>
                        <span className="text-xs text-slate-400">{s.className}</span>
                        {s.lateCnt > 0 && <span className="text-xs font-semibold text-yellow-600">지각 {s.lateCnt}</span>}
                        {s.absentCnt > 0 && <span className="text-xs font-semibold text-red-500">결석 {s.absentCnt}</span>}
                      </div>
                    ))}
                  </div>
                </div>
